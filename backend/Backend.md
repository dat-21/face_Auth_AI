...
Quy Trình:
Kiểm tra user_id đã tồn tại trong DB chưa. Nếu có -> Lỗi 400.
Decode ảnh Base64 sang ảnh OpenCV.
Trích xuất Face Embedding từ ảnh (sử dụng 
get_face_embedding
).
Nếu không tìm thấy mặt -> Lỗi 400.
Lưu document vào MongoDB collection users.
...
Quy Trình:
Decode ảnh Base64 -> OpenCV.
Trích xuất Face Embedding từ ảnh đầu vào (gọi là input_embedding).
Tìm kiếm 1:N (One-to-Many):
...
Tìm user có khoảng cách nhỏ nhất (min_distance).
